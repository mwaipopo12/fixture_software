<?php
  ...
  if (isset($_POST['reg_venue'])) {
    ...
    // Check if a venue with the same name and league type already exists in the database
    $checkQuery = "SELECT COUNT(*) FROM venue WHERE name = ? AND league_type = ?";
    ...
    if ($checkStmt) {
        ...
        if ($count > 0) {
            $err = "Error: A venue with the same name and league type already exists.";
        } else {
            // Venue with the same name and league type does not exist, proceed with insertion
            ...
            if ($insertStmt) {
                ...
                $err = "Error: Statement Preparation Failed (Insert)";
            }
        }
    } else {
